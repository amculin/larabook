                ...
                <form method="POST" action="{{ route('books.update', $book) }}">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-6" style="display: inline-flex">
                            <?php
                            $length = substr($book->dimension, 0, 2);
                            $height = substr($book->dimension, -4, 2);
                            ?>
                            <div class="col-md-4">
                                <input type="number" max="40" min="15" id="dimension_length" class="form-control @error('dimension_length') is-invalid @enderror"
                                    name="dimension_length" value="{{ old('dimension_length', $length) }}" required />
                            </div>
                            ...
                            <div class="col-md-4">
                                <input type="number" max="30" min="15" id="dimension_height" class="form-control @error('dimension_height') is-invalid @enderror"
                                    name="dimension_height" value="{{ old('dimension_height', $height) }}" required />
                            </div>
                        ...
                        <div class="col-md-3">
                            <select required name="publisher_id" id="publisher_id" class="form-control" @error('publisher_id') is-invalid @enderror">
                                ...
                                @foreach($publishers as $id => $name)
                                    <option value="{{ $id }}" {{ $book->publisher_id == $id ? 'selected' : '' }}>{{ $name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-2">
                            <input id="stock" type="number" max="20" class="form-control @error('stock') is-invalid @enderror" name="stock" value="{{ old('stock', $book->stock) }}" required />
